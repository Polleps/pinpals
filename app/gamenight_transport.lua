...
local json = require("app.vendor.json")
local Transport = {}
Transport.__index = Transport

function Transport.new(addr, game, token)
  local host, port = addr:match("^%[([^%]]+)%]:(%d+)$")
  if not host then host, port = addr:match("^([^:]+):(%d+)$") end
...
function Transport:poll(receive)
  ...
  for _ = 1, 64 do
    ...
    if #self.incoming > 1024 * 1024 then return nil, "message too large" end
    if line then
      if #line > 1024 * 1024 then return nil, "message too large" end
      local ok, message = pcall(json.decode, line)
      if not ok or type(message) ~= "table" then return nil, "invalid JSON message" end
      receive(message)
